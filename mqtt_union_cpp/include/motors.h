...
{
    // 왼쪽 모터 방향과 속도 설정
    bool left_is_forward = leftSpeed >= 0.0;
    double abs_left_speed = std::abs(leftSpeed);
    if (abs_left_speed > 1.0) {
        abs_left_speed = 1.0;
    }
    
    // 오른쪽 모터 방향과 속도 설정
    bool right_is_forward = rightSpeed >= 0.0;
    double abs_right_speed = std::abs(rightSpeed);
    if (abs_right_speed > 1.0) {
        abs_right_speed = 1.0;
    }
