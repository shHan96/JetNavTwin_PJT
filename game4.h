...
private:
    ...
    void handleButton(uint8_t button, int16_t value) {
        ...
        switch (button) {
            case 4: // Y - Second joint forward
                button_states.y_pressed = is_pressed;
                break;
            case 0: // A - Second joint backward
                button_states.a_pressed = is_pressed;
                break;
            case 3: // X - Camera pan left
                button_states.x_pressed = is_pressed;
                break;
            case 1: // B - Camera pan right
    ...
    void handleAxis(uint8_t axis, double value) {
        switch (axis) {
            case 1: // Left stick Y - forward/backward
                state.fb_input = -value;
                updateMotors();
                break;
                
            case 0: // Left stick X - rotation
                state.lr_input = -value;
                updateMotors();
                break;
    ...
    void updateMotors() {
        ...
        double left_speed = state.fb_input - state.lr_input;
        double right_speed = state.fb_input + state.lr_input;
